...
public class PatrolReportExportExcelService {
	...
	/*
	 * =========================================================
	 * DYNAMIC PATROL REPORT LAYOUT
	 * =========================================================
	 *
	 * Số cột ảnh không còn cố định là 3.
	 * Mỗi lần export sẽ lấy MAX số ảnh thực tế của từng nhóm
	 * Before / After / HSE trong chính kết quả đã filter.
	 */
	private static class ReportLayout {

		final int beforeImageCount;
		final int afterImageCount;
		final int hseImageCount;

		final List<String> headers;

		final int generalStart;
		final int generalEnd;

		final int beforeStart;
		final int beforeEnd;
		final int createdAt;

		final int picDueStart;
		final int picDueEnd;
		...
		ReportLayout(
			...
			this.beforeImageCount = beforeImageCount;
			this.afterImageCount = afterImageCount;
			this.hseImageCount = hseImageCount;
			...
			result.add("Report ID"); column++;
			result.add("No."); column++;
			result.add("QR Key"); column++;
			result.add("Type"); column++;
			result.add("Group"); column++;
			result.add("Plant"); column++;
			result.add("Division"); column++;
			result.add("Area"); column++;
			result.add("Machine"); column++;

			result.add("Risk Frequency"); column++;
			result.add("Risk Probability"); column++;
			result.add("Risk Severity"); column++;
			result.add("Risk Score"); column++;

			result.add("Comment"); column++;
			result.add("Countermeasure"); column++;
			result.add("Check Information"); column++;
			result.add("Patrol User"); column++;
			...
			for (int i = 1; i <= beforeImageCount; i++) {
				result.add("Before Image " + i);
				column++;
			}

			result.add("Created At");
			createdAt = column++;
			beforeEnd = column - 1;
			...
			result.add("PIC"); column++;
			result.add("Due Date"); column++;
			result.add("Due Date Update Count"); column++;
			result.add("Due Date Updated By"); column++;
			result.add("Due Date Updated At"); column++;
			...
			for (int i = 1; i <= afterImageCount; i++) {
				result.add("After Image " + i);
				column++;
			}

			result.add("After Comment"); column++;
			result.add("After Date"); column++;
			result.add("After PIC"); column++;
			result.add("After Status"); column++;
			result.add("After Assign"); column++;
			...
			result.add("HSE Decision");
			column++;

			for (int i = 1; i <= hseImageCount; i++) {
				result.add("HSE Image " + i);
				column++;
			}

			result.add("HSE Comment"); column++;
			result.add("HSE Date"); column++;
			result.add("HSE User"); column++;
			...
			result.add("Load Status");
			column++;

			systemEnd = column - 1;

			headers = List.copyOf(result);
		}
	}
	...
	private ReportLayout buildReportLayout(
			List<PatrolReportDTO> rows
	) {
		...
		if (rows != null) {
			for (PatrolReportDTO dto : rows) {
				...
				beforeImageCount = Math.max(
						beforeImageCount,
						cleanImages(dto.getImageNames()).size()
				);

				afterImageCount = Math.max(
						afterImageCount,
						cleanImages(dto.getAt_imageNames()).size()
				);

				hseImageCount = Math.max(
						hseImageCount,
						cleanImages(dto.getHse_imageNames()).size()
				);
			}
		}
	...
	private void writePatrolReportsSheet(
		...
		ReportLayout layout = buildReportLayout(rows);
		...
		for (int columnIndex = 0;
		     columnIndex < layout.headers.size();
		     columnIndex++) {

			Cell cell = headerRow.createCell(columnIndex);
			cell.setCellValue(layout.headers.get(columnIndex));
			cell.setCellStyle(
					resolveReportHeaderStyle(
							columnIndex,
							layout,
		...
		for (PatrolReportDTO dto : rows) {
			...
			// =====================================================
			for (int imageIndex = 0;
			     imageIndex < layout.beforeImageCount;
			     imageIndex++) {
			...
			// =====================================================
			for (int imageIndex = 0;
			     imageIndex < layout.afterImageCount;
			     imageIndex++) {
			...
			for (int imageIndex = 0;
			     imageIndex < layout.hseImageCount;
			     imageIndex++) {
		...
		applyReportColumnWidths(sheet, layout);
	}


	private String joinViJp(
			String vi,
			String jp
	) {
		String viText = vi == null ? "" : vi.trim();
		String jpText = jp == null ? "" : jp.trim();
	...
	private CellStyle resolveReportHeaderStyle(
		...
		if (index >= layout.beforeStart
				&& index <= layout.beforeEnd) {
			return styles.headerBlue;
		}

		if (index >= layout.picDueStart
				&& index <= layout.picDueEnd) {
			return styles.headerGreen;
		}

		if (index >= layout.afterStart
				&& index <= layout.afterEnd) {
			return styles.headerGreen;
		}

		if (index >= layout.hseStart
				&& index <= layout.hseEnd) {
			return styles.headerYellow;
		}

		if (index >= layout.systemStart
				&& index <= layout.systemEnd) {
			return styles.headerGrey;
		}
	 ...
	 * Width cho sheet Patrol Reports.
	 */
	private void applyReportColumnWidths(
			Sheet sheet,
			ReportLayout layout
	) {
		// Default width cho toàn bộ layout động.
		...
		// BEFORE IMAGES
		for (int i = 0; i < layout.beforeImageCount; i++) {
			setColumnWidth(sheet, column++, 16);
		}

		setColumnWidth(sheet, column++, 18); // Created At

		// PIC / DUE DATE
		setColumnWidth(sheet, column++, 16); // PIC
		setColumnWidth(sheet, column++, 14); // Due Date
		setColumnWidth(sheet, column++, 12); // Update Count
		setColumnWidth(sheet, column++, 16); // Updated By
		setColumnWidth(sheet, column++, 18); // Updated At

		// AFTER IMAGES
		for (int i = 0; i < layout.afterImageCount; i++) {
			setColumnWidth(sheet, column++, 16);
		}

		// AFTER INFO
		setColumnWidth(sheet, column++, 50); // After Comment
		setColumnWidth(sheet, column++, 18); // After Date
		setColumnWidth(sheet, column++, 16); // After PIC
		setColumnWidth(sheet, column++, 10); // After Status
		setColumnWidth(sheet, column++, 16); // After Assign

		// HSE
		setColumnWidth(sheet, column++, 11); // HSE Decision

		for (int i = 0; i < layout.hseImageCount; i++) {
			setColumnWidth(sheet, column++, 16);
		}

		setColumnWidth(sheet, column++, 50); // HSE Comment
		setColumnWidth(sheet, column++, 18); // HSE Date
		setColumnWidth(sheet, column++, 16); // HSE User

		// SYSTEM
		setColumnWidth(sheet, column, 10); // Load Status
	}
