...
public interface PatrolReportRepo extends JpaRepository<PatrolReport, Long> {
    ...
    @Query(value = """
        ...
        WHERE (:type IS NULL OR LTRIM(RTRIM(type)) = LTRIM(RTRIM(:type)))
          ...
          AND (
                :grp IS NULL
                OR REPLACE(grp, ' ', '') LIKE '%' + REPLACE(:grp, ' ', '') + '%'
          )
        
        
          AND (
                :plant IS NULL
                OR LTRIM(RTRIM(plant)) = LTRIM(RTRIM(:plant))
              )
        
          AND (
                :division IS NULL
                OR LTRIM(RTRIM(division)) LIKE '%' + LTRIM(RTRIM(:division)) + '%'
              )
        
          AND (
                :area IS NULL
                OR LTRIM(RTRIM(area)) LIKE '%' + LTRIM(RTRIM(:area)) + '%'
              )
        
          AND (
                :machine IS NULL
                OR LTRIM(RTRIM(machine)) LIKE '%' + LTRIM(RTRIM(:machine)) + '%'
              )
        
          AND (
                :afStatus IS NULL
                OR LTRIM(RTRIM(at_status)) = LTRIM(RTRIM(:afStatus))
              )
          AND (
                :pic IS NULL
                OR pic COLLATE Vietnamese_CI_AI
                   LIKE '%' + :pic + '%'
              )
        ORDER BY stt DESC
""", nativeQuery = true)
